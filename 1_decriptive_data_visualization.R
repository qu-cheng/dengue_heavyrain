###################################################################################################
## Code for making descriptive plots before fitting models
# 
## Written by Qu Cheng
...
# save_plot("Figures/Fig2.pdf", Fig2, base_height = 7, base_width = 8)
...
GZ.dat <- GZ.daily %>%
  mutate(P8wk = zoo::rollsum(Precipitation/1000, 8*7, align = "right", na.pad = TRUE),    # change mm to m to have smaller values for better estimation by INLA
         P8wk.noself = lag(P8wk, 1),
         HeavyRain = Precipitation > rainfall.threshold
  ) 
